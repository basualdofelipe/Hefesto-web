...
- Issue: `nemea-back/` exists only as empty repo with `.git/`, `.gitignore`, and blank `README.md`. NestJS scaffold not yet created.
- Files: `nemea-back/` (empty)
...
- Fix approach: Execute Phase 2 plan from `.claude/plans/nemea-back/fase-2-scaffold-backend.md` — scaffold NestJS with TypeORM, migrations, Docker, CORS, Helmet, rate limiting.
...
- Files: `nemea-front/src/app/page.tsx`, `nemea-front/.env.example`
...
- Files: `.claude/plans/nemea-back/fase-2-scaffold-backend.md` (line 154), future `nemea-back/src/common/common.module.ts`
...
- Files: `nemea-back/src/entities/` (doesn't exist yet)
...
- Files: `nemea-front/src/app/layout.tsx` (line 48), `nemea-front/src/app/page.tsx` (lines 20-22)
...
- Files: `nemea-front/src/app/__tests__/page.test.tsx`
...
- Files: `nemea-back/` (future config in `src/main.ts`)
...
- Files: Future Phase 3 implementation in `nemea-front/src/app/api/auth/[...nextauth].ts`
...
- Files: Future `nemea-back/docker-compose.yml`
...
- Files: `nemea-front/src/` (future API client)
...
- Files: `nemea-front/src/app/page.tsx` (lines 20-30)
...
- Problem: Frontend has fonts loaded from Google Fonts + local fonts, but no mention of image optimization for product photos (Nemea business = leather products, needs product images)
- Files: `nemea-front/src/app/layout.tsx` (font loading), no `<Image>` component usage yet
...
- Files: `nemea-front/package.json` (lines 33-59)
...
- Files: Future entities in `nemea-back/src/entities/`
...
- Files: `nemea-front/src/app/page.tsx` (lines 8-22)
...
- Files: `nemea-front/eslint.config.mjs` (lines 47-48)
...
- Files: `nemea-front/jest.config.js` (lines 16-20)
...
- Files: `nemea-front/.env.example` (line 2: `NEXT_PUBLIC_API_URL=http://localhost:4000`), `nemea-front/src/app/layout.tsx` metadata
...
- Limit: Railway free tier has limited storage. Nemea V1 has 13 tables with price history (potentially millions of rows for yearly pricing). No archive/cleanup policy.
...
- Files: `nemea-front/package.json` (line 35: `"@tailwindcss/postcss": "^4"`), `nemea-front/tailwind.config.ts` (to be created)
...
- Files: `nemea-front/package.json` (lines 22, 25: `"next": "16.1.6"`, `"react": "19.2.3"`)
...
- Files: Future `nemea-back/package.json` (to be added in Phase 2)
...
- Files: `nemea-front/.husky/pre-commit`, `nemea-front/package.json` (line 50: `"husky": "^9.1.7"`)
...
- Files: `nemea-front/src/` (no `api/` or `hooks/useApi.ts` yet)
...
- Files: `nemea-back/src/` (modules not created yet, but calculadora prototipo exists at root: `calculadora/calculadora-tiendanube.jsx`)
- Risk: Wrong calculations shipped to production. Nemea business model depends on accurate pricing.
- Priority: High — Phase 6 must include unit tests for `calcForward`, `calcInverse`, IVA, IIBB calculations. Reference test data from `Costos Nemea.xlsx`.
...
- Files: Future `nemea-back/src/migrations/` (created in Phase 4 onwards)
...
- Files: Future modules in `nemea-back/src/{supplies,products,config}/`
